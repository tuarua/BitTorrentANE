...
package org.mangui.hls.loader {
    ...
    public class FragmentLoader {
		...
		private var _localFragments:Array = new Array();
		private var _localFilePath:String;
        ...
        private function _keyLoadCompleteHandler(event : Event) : void {
            ...
            if ( _keystreamloader.bytesAvailable == 16 ) {
                ...
                try {
                    ...
                    _hls.dispatchEvent(new HLSEvent(HLSEvent.FRAGMENT_LOADING, _fragCurrent.url, _fragCurrent.seqnum));
        ...
        */
        ...
        private function _fragLoadProgressHandler(event : ProgressEvent) : void {
			...
			*/
			...
			if((event.currentTarget is Loader && event.bytesLoaded > fragData.bytesLoaded)){
				var li:LoaderInfo = event.currentTarget as LoaderInfo;
				trace("BA loader");
				trace(li.bytes);
				//data = li.bytes.readObject();
				data = li.bytes;
				fragData.bytesLoaded += li.bytes.length;
				if (fragData.decryptAES != null) {
					fragData.decryptAES.append(data);
				} else {
					_fragDecryptProgressHandler(data);
				}
			} else if (event.currentTarget is URLStream && event.bytesLoaded > fragData.bytesLoaded && _fragstreamloader.bytesAvailable > 0) {  // prevent EOF error race condition
                
				
				//event.currentTarget.readBytes(data);
        ...
        private function _fragLoadCompleteHandler(event:Event) : void {
			...
			//to mock this
			//create new fragData
			//set the bytes to the loaded bytes, with position of length
			//set bytesLoaded to the length
			//then cop the code below - hope it works
			//may need to create a fragCurrent (or not)
        ...
        private function _loadfragment(frag : Fragment) : void {
            ...
            try {
				...
				//local
				if(_localFragments && _localFragments.length > 0 && _localFragments.indexOf(_fragCurrent.seqnum) > -1){
					CONFIG::LOGGING {
						Log.debug("load fragment from local");
					}
					
					var fle:File = new File(_localFilePath);
					CONFIG::LOGGING {
						Log.debug("loading fragment: " + fle.url);
					}
					_hls.dispatchEvent(new HLSEvent(HLSEvent.FRAGMENT_LOADING, fle.url, frag.seqnum));
					_fragstreamloader.load(new URLRequest(fle.url));
					
				}else{
					CONFIG::LOGGING {
						Log.debug("loading fragment: " + frag.url);
					}
					_hls.dispatchEvent(new HLSEvent(HLSEvent.FRAGMENT_LOADING, frag.url, frag.seqnum));
					_fragstreamloader.load(new URLRequest(frag.url));
					
				}
				
				CONFIG::LOGGING {
					Log.debug("_fragCurrent.seqnum: " + _fragCurrent.seqnum);
				}
        ...
        private function _fragParsingCompleteHandler() : void {
            ...
            try {
                ...
                _loadingState = LOADING_IDLE;
                _ptsAnalyzing = false;
                _hls.dispatchEvent(new HLSEvent(HLSEvent.FRAGMENT_LOADED, _metrics, _fragCurrent.url));
                _fragmentFirstLoaded = true;
                _fragPrevious = _fragCurrent;
            } catch (error : Error) {
		...
		public function setIsLocal(n:int):void {
			trace("setting",n,"to local");
			_localFragments.push(n);
		}
		public function set localFilePath(value:String):void {
